...
$(document).ready(function () {
    //fontyukleme
    setTimeout(function () { loadStyle("https://fonts.googleapis.com/css?family=Poppins:400,500,600,700&display=swap&subset=latin-ext", function () { }); }, 500);
    //sayfaislemleri
...
function KategoriIslemleri() {//Kategori ready sahnesi
    ...
    if (windowidth > 1041) {$('body').on('click','.leftBlock .category-vertical-filters .panel .panel-heading',function(event) {$(this).parent().find('.list-group, .slider-range, .amount').slideToggle(); $(this).toggleClass('active'); }); }
}
...
$(document).on("click",".seClose", function () { $('body').removeClass('overflow transform'); $('.mobilMenu').removeClass('acik'); $('.altMenu').removeClass('active'); $('.ResimliMenu1AltUl').removeClass('active'); $('.mobilMenu .KatMenu1 > li ul').removeClass('active'); $('.mobilMenu .navUl ul').removeClass('active'); $('.mobilMenu .lfMenuAltContent').removeClass('active'); $('.mobilAcilirMenu').html('<i class="fal fa-angle-right"></i>'); $('.mobilaf').removeClass('acik').removeAttr('style'); $('.searchContent').removeClass('active'); $('.welcome').removeClass('active'); $('.CartProduct').removeClass('animated'); $('#lang_flag_container').removeClass('selector'); $('body #divSayfalamaUst .filterBlock').removeClass('active'); });
...
function urunDon() {
    ...
    $( ".productItem").each(function( index ) {if ($(this).find('.divVideoPlayButton').length>0) {$(this).find('.divVideoPlayButton').insertAfter($(this).find('.productDetail')); } });
